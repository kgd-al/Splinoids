...
plot for [i=1:N] for [j=1:S] tmp_i(i,j) using (i + w*bwidth*$2):1 with filledcurve x=i ls -1 notitle,\
     for [i=1:N] for [j=1:S] tmp_i(i,j) using (i + w*bwidth*$2):1 with lines ls -1 notitle,\
     for [i=1:N] for [j=1:S] tmp_i(i,j) using (i - w*bwidth*$2):1 with filledcurve x=i ls -1 notitle,\
     for [i=1:N] for [j=1:S] tmp_i(i,j) using (i - w*bwidth*$2):1 with lines ls -1 notitle,\
     for [i=1:N] for [j=1:S] pointsfile_i(i,j) using (i):1 with boxplot fc "white" notitle, \
     for [j=1:S] NaN with boxes ls -1 title word(cols, j)
