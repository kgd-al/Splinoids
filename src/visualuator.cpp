...
//    else
//      utils::Thrower("No starting state provided. Either provide both an "
//                       "environment and plant genomes or load a previous "
//                       "simulation");
//  }

////  if (!morphologiesSaveFolder.empty() && loadSaveFile.empty())
////    utils::Thrower("Generating morphologies is only meaningful when loading "
////                   "a simulation");
...
//      utils::Thrower("Failed to parse '", duration.substr(1),
//                     "' as a duration (uint)");
